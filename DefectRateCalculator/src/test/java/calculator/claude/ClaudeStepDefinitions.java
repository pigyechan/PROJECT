...
public class ClaudeStepDefinitions {

    private final ClaudeDefectRateCalculator calculator = new ClaudeDefectRateCalculator();
    private ClaudeDefectRateRepository repository;
    private ClaudeDefectRateService service;
    ...
    public void setupDbRecord(long id, int defective, int total) {
        repository = mock(ClaudeDefectRateRepository.class);
        service = new ClaudeDefectRateService(repository, calculator);
        when(repository.findById(id)).thenReturn(new ClaudeProductionRecord(defective, total));
    }
