# critique.py
# 역할: output.json의 content만 받아 약점 3개를 추출한다.
...
CRITIQUE_SYSTEM_PROMPT = Path("prompts/critique_system.md").read_text(encoding="utf-8")
...
def critique(artifact_path: Path, critique_path: Path) -> None:
    ...
    if len(weaknesses) != 3:
        raise ValueError(f"Critique는 약점 3개를 반환해야 합니다. 실제: {len(weaknesses)}개")
